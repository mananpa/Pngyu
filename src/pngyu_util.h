...
inline QString app_home_path() { return QDir::homePath() + "/.pngyu"; }

inline QString app_temporay_path() { return app_home_path() + "/temp"; }

inline bool ma_app_home_path() { return QDir().mkpath(app_home_path()); }
...
inline QString size_to_string_kb(const qint64 size) {
  return QString("%1KB").arg(static_cast<double>(size) / 1024, 0, 'f', 2);
}

inline QString size_to_string_mb(const qint64 size) {
  return QString("%1MB").arg(static_cast<double>(size) / (1024 * 1024), 0, 'f',
                             2);
}
